...
class Ship (GraphObject):
    ...
    def __init__(self, x0, y0, x1, y1, obj_type, screen, filename, turn_flag):
        ...
        self.img = pygame.image.load('skins/' + filename).convert_alpha()
        if turn_flag:
            self.img = pygame.transform.rotate(self.img, 90)
        self.img = pygame.transform.scale(
            self.img, (self.x1 - self.x0, self.y1 - self.y0))
    ...
    def drawShadow(self):
        ...
        self.screen.blit(
            self.img, (position[0], position[1] - (self.y1 - self.y0) / 2))
        self.img.set_alpha(200)
...
class Water (GraphObject):
    ...
    def draw(self):
        ...
        if time_difference > frec:
            ...
            for i in range(Nx):
                for j in range(Ny):
                    rect(self.origin, choice(COLORS),
                         (i * W / Nx, j * H / Ny, W / Nx, H / Ny))
            self.time0 = time.time()

        if 5**(1 + 3 * (time_difference - frec / 2)**2) < 50:
            self.originX = pygame.transform.smoothscale(self.origin,
                                                        (int(W / 5**(1 + 3 * (time_difference - frec / 2)**2)),
                                                         (int(H / 5**(1 + 3 * (time_difference - frec / 2)**2)))))
        else:
            self.originX = pygame.transform.smoothscale(
                self.origin, (int(W / 50), (int(H / 50))))
    ...
    def draw(self):
        ...
        if time_difference > self.frec:
            ...
            self.position_x.append(
                randint(int(-self.end_scale / 4), int(self.end_scale / 4)))
            self.position_y.append(
                randint(int(-self.end_scale / 4), int(self.end_scale / 4)))
            self.stain_size.append(self.begin_scale)
            self.surfaces.append(pygame.Surface(
                (self.end_scale, self.end_scale)))
        ...
        for num in range(len(self.stain_size)):
            self.surfaces[num].set_alpha(
                200 - 200 * self.stain_size[num] / self.end_scale * 2)
            self.surfaces[num].fill('#000000')
            circle(self.surfaces[num],
                   self.stain_color[num],
                   (int(self.end_scale / 2),
                    int(self.end_scale / 2)),
                   self.stain_size[num])
            self.surfaces[num].set_colorkey('#000000')
            self.screen.blit(
                self.surfaces[num],
                (self.x0 + self.position_x[num],
                 self.y0 + self.position_y[num]))
...
class Fire (Smoke):
    def __init__(self, x0, y0, x1, y1, obj_type, screen):
        ...
        self.COLORS = ['#f7943c', '#ffcf48',
                       '#ee9086', '#99958c', '#778899', '#FFFFFF']
        self.frec = 0.5
...
class WaterBlock (GraphObject):
    def __init__(self, x0, y0, x1, y1, obj_type, screen):
        ...
        self.end_scale = min((x1 - x0), (y1 - y0))
        self.speed = 1
        self.COLORS = [(0, 204, 255), (0, 89, 255),
                       (0, 255, 216), (0, 12, 216)]
        self.frec = 3
    ...
    def draw(self):
        ...
        if time_difference > self.frec:
            ...
            self.position_x.append(
                randint(int(-self.end_scale / 2), int(self.end_scale / 2)))
            self.position_y.append(
                randint(int(-self.end_scale / 2), int(self.end_scale / 2)))
            self.stain_size.append(self.begin_scale)
            self.surfaces.append(pygame.Surface(
        ...
        for num in range(len(self.stain_size)):
            self.surfaces[num].set_alpha(
                200 - 200 * self.stain_size[num] / self.end_scale)
            self.surfaces[num].fill('#000000')
            rect(self.surfaces[num], self.stain_color[num], (0, 0, int(
                self.stain_size[num]), int(self.stain_size[num])))
            self.surfaces[num].set_colorkey('#000000')
            self.screen.blit(
                self.surfaces[num],
                (self.x1 /
...
class Button (GraphObject):
    ...
    def draw(self):
        rect(self.screen, '#000080', (self.x0, self.y0,
             self.x1 - self.x0, self.y1 - self.y0))

        font = pygame.font.SysFont('ComicSansMs', 30)
        self.screen.blit(font.render(self.text, 0, '#9932cc'),
                         (self.x0 + self.border, self.y0 + self.border))


class Text (GraphObject):
    def __init__(self, x0, y0, x1, y1, obj_type,
                 screen, text, color='#9932cc'):
        super().__init__(x0, y0, x1, y1, obj_type, screen)
    ...
    def draw(self):
        ...
        self.screen.blit(font.render(
            self.text, 0, self.color), (self.x0, self.y0))
